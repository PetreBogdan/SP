...
f=1/T;
w=2*pi*f;
t = 0:0.001:2*T; %am ales rezolutia temporala de 0.001 s
N=50;
C = zeros(1,2*N+1); %am initializat vectorul de coeficienti cu valori nule
x=(square(w*t,32.5)+abs(square(w*t,32.5)))/2

for n = -N:N

    C(n+N+1) = 1/T * integral(@(t)(1/2*square(w*t,32.5)+1/2*abs(square(w*t,32.5))).*exp(-1j*n*w*t),0,T) ; %calculul oeficientiilor cu formula analitica
...
for n = -N:N
    ...
    yr = yr + C(n+N+1)*exp(1j*n*w*t) ; %am recostruit semnalul initial cu 100 de componente
...
hold off %prin hold on/off pot reprezenta mai multe grafice in acelasi sistem
figure(2);
hold on

stem((-N:N)*w,2*abs(C));%functia stem este utilizata pentru reprezentarea functilor sau a
